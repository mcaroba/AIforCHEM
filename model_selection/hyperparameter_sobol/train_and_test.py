...
sampler = qmc.Sobol(d=3, scramble=False) # 3D Sobol sequence
sample = sampler.random_base2(m=6) # 2**m samples
####################################################################################
...
results = []
for num in sample:
    n1 = int(100. + num[0] * 300.)
    n2 = int(100. + num[1] * 300.)
    alpha = 10.**(-5. + num[2]*3.)
    ...
    t_end = time.time()
    t_test = t_end - t_beg
    print(n1, n2, alpha, t_train, t_test, RMSE_test)
    results.append([n1, n2, alpha, t_train, t_test, RMSE_test])

results = np.array(results)

idx = np.argmin(results[:,5])
print("Parameters for the lowest RMSE: ", *results[idx])
