...
# Select 20% for testing, 20% for validation and the rest for training
random_list = list(range(0,len(desc)))
np.random.shuffle(random_list)
desc_test = [desc[i] for i in random_list[0:int(0.2*len(desc))]]
desc_validation = [desc[i] for i in random_list[int(0.2*len(desc)):int(0.4*len(desc))]]
desc_train = [desc[i] for i in random_list[int(0.4*len(desc)):len(desc)]]
...
results = []
for n1 in [100, 200, 300, 400]:
    for n2 in [100, 200, 300, 400]:
        for alpha in [0.00001, 0.0001, 0.001, 0.01]:
            # This builds the neural network object
            encoder_hidden_layers = [n1, n2, 50]
            hidden_layers = []
            ...
            t_start = time.time()
            RMSE_validation = 0.
            for i in range(0, len(desc_validation)):
                v = autoencode(weights, bias, desc_validation[i])
                RMSE_validation += np.dot(desc_validation[i]-v, desc_validation[i]-v)
            RMSE_validation = np.sqrt(RMSE_validation/len(desc_validation))
            t_end = time.time()
            t_validation = t_end - t_beg
            # Testing
            t_start = time.time()
            RMSE_test = 0.
            ...
            t_end = time.time()
            t_test = t_end - t_beg
            print(n1, n2, alpha, t_train, t_validation, t_test, RMSE_validation, RMSE_test)
            results.append([n1, n2, alpha, t_train, t_validation, t_test, RMSE_validation, RMSE_test])

results = np.array(results)

idx = np.argmin(results[:,6])
print("Parameters for the lowest RMSE according to validation set: ", *results[idx])
